...
async def process_questions(message: Message):
    ...
    if len(questions) == 0:
        ...
        await message.answer('Klausimu nera.')
        ...
        for cid, question in questions:
            ...
            markup.add(InlineKeyboardButton(
                'Atsakyti', callback_data=question_cb.new(cid=cid, action='answer')))
...
async def process_answer(query: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await query.message.answer('Irasykite atsakyma.', reply_markup=ReplyKeyboardRemove())
...
async def process_submit(message: Message, state: FSMContext):
    ...
    await message.answer('Siusti?.', reply_markup=submit_markup())
...
async def process_send_answer(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await message.answer('Issiusta!', reply_markup=ReplyKeyboardRemove())
