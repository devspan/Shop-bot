...
async def process_cart(message: Message, state: FSMContext):
    ...
    if len(cart_data) == 0:
        ...
        if order_cost != 0:
            ...
            markup.add('📦 Suformuoti uzsakyma')

            await message.answer('Pereiti prie uzsakymo pateikimo?',
                                 reply_markup=markup)
    ...
    if 'count' == action:
        ...
        async with state.proxy() as data:
            ...
            if 'products' not in data.keys():
                ...
                await query.answer('Kiekis - ' + data['products'][idx][2])
...
@dp.message_handler(IsUser(), text='📦 Suformuoti uzsakyma')
...
async def checkout(message, state):
    ...
    await message.answer(f'{answer}\nBendra uzsakymo kaina: {total_price}₽.',
