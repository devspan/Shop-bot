...
class DatabaseManager(object):
    ...
    def create_tables(self):
        ...
        #self.query('DROP TABLE products')
        self.query('CREATE TABLE IF NOT EXISTS products (idx text, title text, body text, photo blob, price int, tag text, stock INT DEFAULT 1)')
        self.query('CREATE TABLE IF NOT EXISTS orders (id INTEGER PRIMARY KEY AUTOINCREMENT, cid int, usr_name text, usr_address text, products text, status TEXT DEFAULT \'pending\', order_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP, order_time TIMESTAMP)')
        ...
        self.query('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, cid int, username TEXT, notes TEXT, completed_orders_count INT DEFAULT 0, ban BOOLEAN DEFAULT false)')
