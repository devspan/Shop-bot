...
cancel_message = '❌ Cancel'  # Text for cancel button
...
async def process_settings(message: Message):
    ...
    await message.answer('⚙️ Category Settings:', reply_markup=markup)
...
async def category_callback_handler(query: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await query.answer('All products in category.')
...
async def delete_category_handler(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        if 'category_index' in data.keys():
            ...
            await message.answer('Added!', reply_markup=ReplyKeyboardRemove())
            await process_settings(message)
...
async def process_body_back(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await message.answer(f"Go back to <b>{data['title']}</b>?", reply_markup=back_markup())
...
async def process_image_url(message: Message, state: FSMContext):
    ...
    if message.text == back_message:
        ...
        async with state.proxy() as data:
            ...
            await message.answer(f"Go back to <b>{data['body']}</b>?", reply_markup=back_markup())
...
async def process_price_invalid(message: Message, state: FSMContext):
    ...
    if message.text == back_message:
        ...
        async with state.proxy() as data:
            ...
            await message.answer("Different photo?", reply_markup=back_markup())
...
async def show_products(m, products, category_idx):
    ...
    await m.answer('❔ Would you like to add or delete something?', reply_markup=markup)
